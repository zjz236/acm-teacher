<template>
	<div class="program-topic">
		<el-card :header="`${common?'公共':'个人'}题库-程序设计题`" v-loading="tableLoading">
			<div class="add-search" v-if="!editDisable">
				<el-tooltip class="item" effect="dark" content="添加程序设计题" placement="top">
					<el-button
		...
		<program-modify :disabled="editDisable" :loading="formLoading" v-model="visible" :data="formData"
										:editable="editable"
										@save="topicModify"></program-modify>
		<program-test :disabled="editDisable" program-type="programTestData" :program-id="topicId" :test-data="testDataInfo"
...
<script>
  ...
  export default {
    ...
    computed: {
      common() {
        return this.$route.name === 'commonProgram'
      },
      ...mapState([
        'userInfo'
      ]),
    ...
    methods: {
      async deleteGapTest() {
        try {
          ...
          if (result.code) {
            this.testShow({topicId: this.topicId})
          }
        } catch (e) {
          console.error(e)
        }
