<template>
    <div class="public-bank">
        <div class="title"><h2>查看公共题库</h2><a v-if="teacherInfo.is_admin"
        ...
        <div class="bank-content">
            ...
            <Table :columns="columns" :data="datas" border>
                <template slot-scope="{row,index}" slot="operation">
                    <a v-if="teacherInfo.is_admin" class="read" @click="deleteCorfirm(row.Id)">删除</a>
                    <a class="read" @click="updateSome(row.Id)">{{teacherInfo.is_admin?'修改':'查看'}}</a>
                </template>
        ...
        <yes-no :userInfo="teacherInfo" :publicc="true" :modalShow="tfshow" v-bind:show.sync="tfshow" :getList="getYesNOList" :Id="tfId"></yes-no>
        <up-select :userInfo="teacherInfo" :publicc="true" :modalShow="selectshow" v-bind:show.sync="selectshow" :getList="getSelectList"
                   :Id="selectId"></up-select>
        <up-gap :userInfo="teacherInfo" :publicc="true" :modalShow="gapshow" v-bind:show.sync="gapshow" :getList="getGapList" :Id="gapId"></up-gap>
        <up-pro :userInfo="teacherInfo" :publicc="true" :modalShow="proshow" v-bind:show.sync="proshow" :getList="getProgramList" :Id="proId"></up-pro>
        <gap-test v-if="teacherInfo.is_admin" :modalShow="gapTestShow" v-bind:show.sync="gapTestShow" :Id="gapId" :getList="getGapList"></gap-test>
        <pro-test v-if="teacherInfo.is_admin" :modalShow="proTestShow" v-bind:show.sync="proTestShow" :Id="proId" :getList="getProgramList"></pro-test>
    </div>
</template>
            ...
            dataFocus(value) {
                switch (value) {
                    ...
                    case 2:
                        ...
                        break
                    case 3:
                        this.columns = (this.teacherInfo.is_admin? this.adminGapColumns:this.otherGapColumns)
                        this.currentPage = 1
                        this.search.id = 'Id'
                        this.search.sort = 0
                        this.getGapList()
                        break
                    case 4:
                        this.columns =(this.teacherInfo.is_admin? this.adminProgramColumns:this.otherProgramColumns)
                        this.currentPage = 1
                        this.search.id = 'Id'
                        this.search.sort = 0
                        this.getProgramList()
