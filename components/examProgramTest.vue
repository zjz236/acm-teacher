    ...
    <Modal v-model="show" :width="900" title="修改测试数据" @on-cancel="cancelModal">
        <div class="show-content">
            <Form :loading="formLoad" ref="testValidate" label-position="right" :model="testValidate"
                  :label-width="110">
                <FormItem label="程序设计题编号"><Input disabled v-model="testValidate.programId"></Input></FormItem>
    ...
    export default {
        ...
        methods: {
            ...
            deleteProgramData(index){
                ...
                ajaxService.deleteExamProgramData({Id,programId,examId:this.examId}).then(res=>{
                    if (res.code==1){
            ...
            updateData(){
                ...
                ajaxService.updateExamProgramData(formdata).then(res => {
                    if (res.code == 1) {
                        this.formLoad = false
                        this.$Message.success(res.msg)
                        this.getExamProgramData()
            ...
            submitData() {
                ...
                formdata.append("inputData", inputData)
                formdata.append("outputData", outputData)
                ajaxService.addExamProgramData(formdata).then(res => {
                    if (res.code == 1) {
                        this.formLoad = false
                        this.$Message.success(res.msg)
                        this.getExamProgramData()
            ...
            getExamProgramData() {
                this.tableLoad = true
                ajaxService.getExamProgramData({programId: this.Id,examId:this.examId}).then(res => {
                    if (res.code == 1) {
