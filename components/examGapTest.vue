        ...
        <div class="show-content">
            ...
            <Table :loading="tableLoad" border :columns="testColumns" :data="testData">
                <template slot-scope="{row,index}" slot="inputData">
                    <a :href="'http://127.0.0.1:1236/download/template?file='+gapTestData[index].inputData"
                ...
                <template slot-scope="{row,index}" slot="outputData">
                    <a :href="'http://127.0.0.1:1236/download/template?file='+gapTestData[index].outputData"
    ...
    export default {
        ...
        methods: {
            ...
            deleteGapData(index){
                ...
                ajaxService.deleteExamGapData({Id,gapId,examId:this.examId}).then(res=>{
                    if (res.code==1){
            ...
            updateData(){
                ...
                ajaxService.updateExamGapData(formdata).then(res => {
                    if (res.code == 1) {
                        this.formLoad = false
                        this.$Message.success(res.msg)
                        this.getExamGapData()
            ...
            submitData() {
                ...
                formdata.append("inputData", inputData)
                formdata.append("outputData", outputData)
                ajaxService.addExamGapData(formdata).then(res => {
                    if (res.code == 1) {
                        this.formLoad = false
                        this.$Message.success(res.msg)
                        this.getExamGapData()
            ...
            getExamGapData() {
                this.tableLoad = true
                ajaxService.getExamGapData({gapId: this.Id,examId:this.examId}).then(res => {
                    if (res.code == 1) {
