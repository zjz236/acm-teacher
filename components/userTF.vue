<template>
    <Modal :width="900" v-model="modalShow" title="从个人题库选题"  @on-cancel="cancelModal">
        ...
        <Table :columns="TFColumns" :data="TFDatas" @on-selection-change="selectChange" :loading="load">
            <template slot-scope="{row,index}" slot="section">
                <span>{{getSection(row.section)}}</span>
            </template>
            <template slot-scope="{row,index}" slot="operation">
                <a @click="addExamTF([row.Id])" v-if="!row._disabled">添加</a>
                <span v-else>已添加</span>
            </template>
        </Table>
        </div>
        <div slot="footer"><Button type="primary" @click.native="addExamTF(selectId)">添加</Button></div>
    </Modal>
</template>
    ...
    export default {
        ...
        methods:{
            ...
            addExamTF(data){
                ...
                ajaxService.addExamTF({public:0,examId:this.examId,data}).then(res=>{
                    if (res.code==1){
                        this.getUserTF()
                        this.getList()
                    }
            ...
            getUserTF(){
                this.load=true
                ajaxService.getUserTF({examId:this.examId,section:this.search,limit:this.pageSize,page:this.currentPage-1,public:0}).then(res=>{
                    if (res.code==1){
