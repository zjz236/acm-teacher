<template>
    <Modal v-model="show" :width="900" title="修改我的程序设计题" @on-cancel="cancelModal">
        ...
        <div class="submit-footer" slot="footer"><Button type="primary" @click.native="updateExamProgramInfo('topicValidate')">提交</Button></div>
    </Modal>
</template>
...
<script>
    ...
    export default {
        ...
        methods:{
            ...
            getExamProgramInfo(){
                ajaxService.getExamProgramInfo({Id:this.Id,examId:this.examId}).then(res=>{
                    if (res.code==1){
            ...
            updateExamProgramInfo(name){

                this.$refs[name].validate((valid) => {
                    if (valid) {
                        let data=this.topicValidate
                        data.examId=this.examId
                        ajaxService.updateExamProgramInfo(data).then(res=>{
                            if (res.code==1){
