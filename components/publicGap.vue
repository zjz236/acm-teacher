        ...
        <div class="public-gap">
            ...
            <Table :columns="SelectColumns" :data="SelectDatas" @on-selection-change="selectChange" :loading="load">
                ...
                <template slot-scope="{row,index}" slot="operation">
                    <a @click="addExamGap([row.Id])" v-if="!row._disabled">添加</a>
                    <span v-else>已添加</span>
                </template>
            </Table>
        </div>
        <div slot="footer"><Button type="primary" @click.native="addExamGap(selectId)">添加</Button></div>
    </Modal>
</template>
    ...
    export default {
        ...
        methods:{
            ...
            addExamGap(data){
                ...
                ajaxService.addExamGap({public:1,examId:this.examId,data}).then(res=>{
                    if (res.code==1){
                        this.getUserGap()
                        this.getList()
                    }
            ...
            getUserGap(){
                this.load=true
                ajaxService.getUserGap({examId:this.examId,section:this.search,limit:this.pageSize,page:this.currentPage-1,public:1}).then(res=>{
                    if (res.code==1){
